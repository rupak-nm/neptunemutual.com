...
function hljsDefineSolidity (hljs) {
  ...
  const numSizes = byteSizes.map((bytes) => { return bytes * 8 })
  const precisions = []
  for (i = 0; i <= 80; i++) {
    precisions[i] = i
  }

  const bytesTypes = byteSizes.map((size) => { return 'bytes' + size })
  const bytesTypesString = bytesTypes.join(' ') + ' '

  const uintTypes = numSizes.map((size) => { return 'uint' + size })
  const uintTypesString = uintTypes.join(' ') + ' '

  const intTypes = numSizes.map((size) => { return 'int' + size })
  ...
  const fixedTypes = sizePrecisionPairs.map((pair) => { return 'fixed' + pair })
  const fixedTypesString = fixedTypes.join(' ') + ' '

  const ufixedTypes = sizePrecisionPairs.map((pair) => { return 'ufixed' + pair })
